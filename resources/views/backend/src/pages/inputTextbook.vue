...
<div>
  <b-row>
    <b-colxx xxs="12">
      <piaf-breadcrumb :heading="'Textbook'" />
  ...
  <b-row>
    <b-colxx xxs="12">
      <b-card class="mb-4">
        ...
        <!-- Modal Form -->
        <b-modal size="xl" title="Input Textbook" hide-footer centered id="modal-form">
          <b-form @submit.prevent="doSave()">
            <b-row>
              <b-col xs=12 md=4>
                <b-form-group description="click to add or remove image" label="Foto">
                  <label for="form-cover" class="w-100 border">
                    <b-img :src="(form.data.cover) ? form.data.cover : require('../assets/img/default.png')" fluid class="cursor-pointer w-100" />
                    <input type="file" ref="form-cover" hidden id="form-cover" @change="previewImage" accept="image/*">
                  </label>
                </b-form-group>
              </b-col>
              <b-col xs=12 md=8>
                <b-row>
                  <b-col xs=12 sm=6>
                    <b-form-group label="Judul Buku" :state="!$v.form.data.judul.$error">
                      <b-input v-model="$v.form.data.judul.$model"/>
                    </b-form-group>
                  </b-col>
                  <b-col xs=12 sm=6>
                    <b-form-group label="Pengarang" :state="!$v.form.data.pengarang.$error">
                      <b-input v-model="$v.form.data.pengarang.$model"/>
                    </b-form-group>
                  </b-col>
                  <b-col xs=12 sm=6>
                    <b-form-group label="ISBN" :state="!$v.form.data.isbn.$error">
                      <b-input v-model="$v.form.data.isbn.$model"/>
                    </b-form-group>
                  </b-col>
                  <b-col xs=12 sm=6>
                    <b-form-group label="Tahun Terbit" :state="!$v.form.data.tahun_terbit.$error">
                      <b-input v-model="$v.form.data.tahun_terbit.$model"/>
                    </b-form-group>
                  </b-col>
                  <b-col xs=12 sm=6>
                    <b-form-group label="Edisi" :state="!$v.form.data.edisi.$error">
                      <b-input v-model="$v.form.data.edisi.$model"/>
                    </b-form-group>
                  </b-col>
                  <b-col xs=12 sm=6>
                    <b-form-group label="Penerbit" :state="!$v.form.data.penerbit.$error">
                      <b-input v-model="$v.form.data.penerbit.$model"/>
                    </b-form-group>
                  </b-col>
                  <b-col xs=12 sm=6>
                    <b-form-group label="Kota" :state="!$v.form.data.kota.$error">
                      <b-input v-model="$v.form.data.kota.$model"/>
                    </b-form-group>
                  </b-col>
                  <b-col xs=12 sm=6>
                    <b-form-group label="Kategori" :state="!$v.form.data.kategori.$error">
                      <b-select v-model="$v.form.data.kategori.$model" :options="opt.kategori"/>
                    </b-form-group>
                  </b-col>
                </b-row>
              </b-col>
...
export default {
  ...
  methods: {
    ...
    doUpdate(id){
      let url = `${apiUrl}/api/textbook/get`
      let payload = {'id': id}
      axios
      .post(url,payload)
      .then(res => {
    ...
    doSave(){
      ...
      let url = `${apiUrl}/api/textbook/create`
      if (this.form.data.id)
        url = `${apiUrl}/api/textbook/update`
    ...
    doDelete(id, doDelete) {
      ...
      if (doDelete) {
        let url = `${apiUrl}/api/textbook/delete`
        let payload = {'id': id}
        axios
        .post(url,payload)
        .then(res => {
    ...
    previewImage(event) {
      ...
      reader.onload = function () {
        vm.form.data.cover = reader.result
      };
      reader.onerror = function () {
        vm.form.data.cover = null
      };
    },
    clearForm() {
