                ...
                @if(!empty($topic))
                    ...
                    @foreach($topic as $key => $topicVal)
                        <tr id="T{$i}"> 
                            ...
                            <td class="row-index text-center"> 
                                <textarea required class = "form-control" name = "topic[{{$i}}][topic]" >{{ $key }}</textarea>
                            </td> 
                            <td class="row-index text-center">
                                <div class="row"> 
                            ...
                            <td class="row-index text-center"> 
                                <table width="100%" id="sub_topik_data-{{$i}}">
                                    ...
                                    @foreach($topicVal as $keySub => $v)
                                        ...
                                        @endif
                                            <td>
                                                <textarea class="form-control mb-3" type="text" name="topic[{{$i}}][sub_topik][]" >{{ $v['sub_topic'] }}</textarea>
                                            </td>
                                            <td>
                                                @if($keySub != 0)
...
<script>
    ...
    $(document).ready(function(){
        ...
        $("#button_topik").on("click",function(){
            ...
            $("#cp_tbody .cp_text").each(function(){
           ...
           $('#topik_tbody').append(`<tr id="T${rowIdx}"> 
                    ...
                    <td class="row-index text-center"> 
                        <textarea type = "text" class = "form-control" required name = "topic[${rowIdx}][topic]"></textarea>
                    </td> 
                    <td class="row-index text-center">
                        <div class="row"> 
                            <div class="col-md-10 pr-0">
                    ...
                    <td class="row-index text-center"> 
                        <table width="100%" id="sub_topik_data-${rowIdx}">
                            <tr>
                                <td>
                                    <textarea class="form-control mb-3" type="text" name="topic[${rowIdx}][sub_topik][]"></textarea>
                                </td>
                                <td>
                                </td>
                            </tr>
    ...
    function addSubTopic(row) {
        ...
        var html = `<tr id="sub_topik_add-${subTopicIndex}">
                        <td>
                            <textarea class="form-control mb-3" type="text" name="topic[${row}][sub_topik][]" ></textarea>
                        </td>
                        <td>
